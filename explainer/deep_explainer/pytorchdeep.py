import numpy as np
from explainer._explainer import Explainer
from deep_explainer.deep_utils import _check_additivity
...
class PyTorchDeep(Explainer):
    def __init__(self, model, data):
        ...
        self.interim = False # Sub layer
        self.interim_inputs_shape = None
        self.expected_value = None # to keeep the DeepExplainer base happy
        if isinstance(model ,tuple):
            ...
            # if we are taking an iterim layer, the 'data' is going to be the input of that interim layer, we will capture this using a forward hook
            with torch.no_grad():
                _ = model(*data)  # * = unpacking the list, for instance if data = [a,b] then *data = a,b  => model(a,b)   
                interim_inputs = self.layer.target_input  
                if type(interim_inputs) is tuple:
                    # this should always be true, but just to be safe
                    self.interim_inputs_shape = [x.shape for x in interim_inputs]
                else:
